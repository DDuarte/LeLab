...
public:
    ...
    Quaternion(const Vector<3, T>& vec, T angle = 1) : X(vec.GetX()), Y(vec.GetY()), Z(vec.GetZ()), W(angle) {}
    ...
    void Set(const Vector<3, T>& vec) { X = vec.GetX(); Y = vec.GetY(); Z = vec.GetZ(); }
    void Set(const Vector<3, T>& vec, T w) { Set(vec); W = w; }
    ...
    Vector<3, T> operator *(const Vector<3, T>& vec) const
    {
        ...
        return Vector<3, T>(other.X, other.Y, other.Z);
    }
